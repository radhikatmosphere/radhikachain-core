...
{
    // Check proof of work/karma matches claimed amount
    if (fCheckPOW && !CheckProofOfKarmaHeader(block, consensusParams))
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-hash", "proof of karma failed");
...
bool HasValidProofOfWork(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams)
{
    return std::all_of(headers.cbegin(), headers.cend(),
            [&](const auto& header) { return CheckProofOfKarmaHeader(header, consensusParams);});
}
